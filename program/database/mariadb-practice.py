...
if __name__ == '__main__':
    ...
    """
    if not Path(f'{PROJECT_DIR.as_posix()}/{key}-{value}.csv').exists():
        Path(f'{PROJECT_DIR.as_posix()}/{key}-{value}.csv').touch(exist_ok=True)
        print(fg.RED,Path(f'{PROJECT_DIR.as_posix()}/{key}-{value}.csv').as_posix(),fg.END)
        print(fg.DARKYELLOW,Path(f'{PROJECT_DIR.as_posix()}/{key}-{value}.csv').exists(),fg.END)
    
    for w in  [ v for v in cur.fetchall() ]:
        with open(f'{PROJECT_DIR.as_posix()}/{key}-{value}.csv', mode='a', encoding='utf-8') as fp:
            fp.write(w + "\n")
    """
